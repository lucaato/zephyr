...
static void after(void *arg)
{
	...
	/* only valid pointers to char array must be stored in these maps, anything else will break */
	//(void) sys_hashmap_clear(&map, string_map_free_callback, NULL);
	//(void) sys_hashmap_clear(&custom_load_factor_map, string_map_free_callback, NULL);
	// TODO only use in specific test that checks for dynamically allocated strings
