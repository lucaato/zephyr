...
SYS_HASHMAP_DEFAULT_DEFINE_ADVANCED(map, hash_string, eq_string,
...
int string_map_insert(struct sys_hashmap *map, const char *key, uint64_t value, uint64_t *old_value, char **old_key) {
    uint64_t uint_old_key = 0;
    int res = 0;

    res = sys_hashmap_insert(map, POINTER_TO_UINT(key), POINTER_TO_UINT(value), old_value, &uint_old_key);
...
// not the best encoding but it works :D
...
static uint32_t hash_string(const void *uint64_ptr_key, size_t len) {
    __ASSERT(len == sizeof(uint64_t), "Received an invalid length for the hasmap key, expected only uint64_t");
